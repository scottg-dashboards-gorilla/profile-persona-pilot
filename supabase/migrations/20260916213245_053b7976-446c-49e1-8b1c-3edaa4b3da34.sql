  ...
  IF NEW.released_at IS NOT NULL AND OLD.released_at IS NULL THEN
    ...
    IF NEW.comp_approval_status <> 'approved' THEN
      RAISE EXCEPTION 'Cannot share the outcome: HR must sign off on the pay outcome first.'
        USING ERRCODE = 'check_violation';
    END IF;
    IF NEW.connect_held_at IS NULL OR COALESCE(btrim(NEW.connect_note), '') = '' THEN
      RAISE EXCEPTION 'Cannot share the outcome: log the connect conversation with the employee and add a note first.'
        USING ERRCODE = 'check_violation';
    END IF;
  END IF;
  RETURN NEW;
